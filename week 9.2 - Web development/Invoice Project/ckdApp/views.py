...
class dataUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            data_up= request.POST.get('unit_price')
            data_qu=request.POST.get('quantity')
            data_vat=request.POST.get('vat')
            data_cogs=request.POST.get('cogs')
            data_gi=request.POST.get('gross_income')
            #print (data)
            ...
            data = np.array([data_up,data_qu,data_vat,data_cogs,data_gi])
            #sc = StandardScaler()
...
#data_for_prediction =obj.pca(np.array(data_for_prediction),y_test)
            ...
            return render(request, "succ_msg.html", {'data_up':data_up,'data_qu':data_qu,'data_vat':data_vat,'data_cogs':data_cogs,'data_gi':data_gi,
                                                        'out':out})
